...
class Container:
    ...
    def add_cargo_to_container(self,cargo:Cargo):
        ...
        max_score=99999999999999999999
        for i in range(length):
            x,y,z=self.available[i].tuple
            cargo.set_locate_point(x,y,z)
            for j in range(6):
                cargo.change_pose(j)
                result1=self.check_crash_and_suspended(cargo)
                ...
                if(result1==False and result2==False):
                    if(cargo.get_z==0):#找到使得tmp_score最小的摆放方式
                        ...
                        tmp_score=self.alpha*(self.current_bottom_area)*(1-self.alpha)*max(cargo.get_z+cargo.load_height,self.current_max_height)
                        if(tmp_score<max_score):
                            storage_result[0]=x
                            storage_result[1]=y
                            storage_result[2]=z
        ...
        if(max_score==99999999999999999999):
            return False
        else:
...
if __name__ == '__main__':
    #test_data=np.round(np.random.normal(20, 5, size=(150, 3)))
    #print(test_data)
    ...
    plt.figure()
    plt.plot([i for i in range(150)],collect)
